...
export const PinnedTemplates: FC<Props> = ({ position }) => {
  ...
  useEffect(() => { const el = scrollRef.current; if (!el) return; check(); const t = setTimeout(check, 150); el.addEventListener('scroll', check, { passive: true }); window.addEventListener('resize', check); const ro = new ResizeObserver(check); ro.observe(el); return () => { clearTimeout(t); el.removeEventListener('scroll', check); window.removeEventListener('resize', check); ro.disconnect(); }; }, [check, pinnedTemplateIds]);
